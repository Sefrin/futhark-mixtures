...
-- remember to normalize the data before
module spherical_k_means (dataset: mixture_dataset): (mixture with data = dataset.data_rep) = {
    module V = dataset.V
    module I = dataset.I
    module C = i32
    type val_t = V.t
    ...
    let maximize [k] (X: data) (_: theta_sized [k][]) (posts: posteriors): theta = 
        ...
        let centers = map2 (\center count -> map (\x -> x V./ (V.i64 (I.to_i64 (if count I.== i0 then I.i32 1 else count)))) center) cluster_sums center_counts
        
        let cluster_magnitudes = map (\c -> 
                            let c_squared = map (\x -> x V.* x) c
                             in V.sqrt (V.sum c_squared))
                            centers
            in map2 (\center center_magnitude -> 
                                        map (\x -> x V./ center_magnitude) center
